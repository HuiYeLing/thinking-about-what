...
# 将命中概率转换为百分比
probabilities_percentage = [p * 100 for p in probabilities]
max_probability_percentage = max_probability * 100

# 绘制结果
plt.plot(H_values, probabilities_percentage, label='命中概率 (%)')
plt.axvline(optimal_H, color='r', linestyle='--', label=f'最佳引爆深度: {optimal_H:.2f} m')
plt.xlabel('引爆深度 (m)',fontsize=20)
plt.ylabel('命中概率 (%)',fontsize=20)
...
print(f'最大命中概率: {max_probability_percentage:.2f} %')